...
class CodeMirror {
    ...
    setSelections(p, primIndex) {
        var doc = this.cm6.state.doc;
        var ranges = p.map(x => {
            return state.EditorSelection.range(indexFromPos(doc, x.anchor), indexFromPos(doc, x.head));
        });
        this.cm6.dispatch({
    ...
    setSelection(anchor, head, options) {
        var doc = this.cm6.state.doc;
        var ranges = [state.EditorSelection.range(indexFromPos(doc, anchor), indexFromPos(doc, head))];
        this.cm6.dispatch({
            selection: state.EditorSelection.create(ranges, 0)
        });
    ...
    findPosV(start, amount, unit, goalColumn) {
        ...
        let range = state.EditorSelection.range(startOffset, startOffset, goalColumn);
...
function getBase(view$1) {
    let rect = view$1.scrollDOM.getBoundingClientRect();
    let left = view$1.textDirection == view.Direction.LTR ? rect.left : rect.right - view$1.scrollDOM.clientWidth;
    return { left: left - view$1.scrollDOM.scrollLeft, top: rect.top - view$1.scrollDOM.scrollTop };
}
...
const vimPlugin = view.ViewPlugin.fromClass(class {
    constructor(view$1) {
        ...
        this.view = view$1;
        const cm = (this.cm = new CodeMirror(view$1));
        Vim.enterVimMode(this.cm);
        this.view.cm = this.cm;
        this.cm.state.vimPlugin = this;
        this.blockCursor = new BlockCursorPlugin(view$1, cm);
        this.updateClass();
